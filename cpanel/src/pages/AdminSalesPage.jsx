...
function OrderDetailDialog({ company, employees, language, labels, onClose, order }) {
  const money = currencyFormatter(company, language);
  const assigned = employees.find((employee) => String(employee.id) === String(order.assignedToEmployeeId || order.handledByEmployeeId));
  const created = order.createdAt && !Number.isNaN(new Date(order.createdAt).getTime()) ? new Date(order.createdAt).toLocaleString(language) : "—";
  return <div className="sales-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()} role="presentation"><section aria-modal="true" className="sales-modal sales-order-detail" role="dialog">
    <div className="sales-modal-heading"><div><span className="sales-eyebrow">{labels.orderReference}</span><h2>{order.id}</h2></div><button aria-label={labels.cancel} className="sales-modal-close" onClick={onClose} type="button"><X size={19} /></button></div>
    <div className="sales-order-detail-grid"><div><span>{labels.customer}</span><strong>{order.customer?.name || "—"}</strong><small>{order.customer?.phone || order.customer?.email || "—"}</small></div><div><span>{labels.status}</span><strong>{order.status || "—"}</strong></div><div><span>{labels.paymentMethod}</span><strong>{order.paymentMethod || "—"}</strong></div><div><span>{labels.date}</span><strong>{created}</strong></div><div><span>{language === "ar" ? "أنشئ بواسطة" : "Created by"}</span><strong>{order.createdByEmployeeName || order.createdBy?.name || order.createdBy?.role || "—"}</strong></div><div><span>{language === "ar" ? "الموظف المعيّن" : "Assigned employee"}</span><strong>{assigned?.name || "—"}</strong></div></div>
    <div className="sales-order-lines"><h3>{language === "ar" ? "عناصر الطلب" : "Line items"}</h3>{(order.items || []).map((item, index) => <div key={`${item.productId || item.slug}-${index}`}><span>{item.productName || item.name || item.slug || item.productId}<small>{[item.selectedSize || item.size, item.selectedColor || item.colorName].filter(Boolean).join(" · ")}</small></span><bdi dir="ltr">{item.quantity || 1} × {money(item.price)}</bdi><strong><bdi dir="ltr">{money(item.lineTotal ?? Number(item.price || 0) * Number(item.quantity || 1))}</bdi></strong></div>)}</div>
    <div className="sales-order-detail-total"><span>{labels.amount}</span><strong><bdi dir="ltr">{money(order.total)}</bdi></strong></div>
  </section></div>;
}
...
function OrdersPage({ canAdd, company, employees, language, labels, onAdd, onAssignEmployee, onDeleteOrder, onNavigate, onStatusChange, onUnsupported, orders, products, t, user }) {
  ...
  const filtered = filterSalesOrders(orders, { customer, from, query, status, to });
  const customers = [...new Map(orders.map((order) => [String(order.customer?.email || order.customer?.phone || order.customer?.id || order.customer?.name || ""), order.customer]).filter(([key]) => key)).entries()];
  ...
  return <>
    ...
    <section className="sales-data-card sales-orders-card"><div className="sales-card-heading"><div><h2>{labels.orders}</h2></div></div>
      <SalesToolbar count={filtered.length} labels={labels} query={query} setQuery={setQuery}><select aria-label={labels.status} onChange={(event) => setStatus(event.target.value)} value={status}><option value="all">{labels.allStatuses}</option>{["Pending", "Processing", "Completed", "Cancelled"].map((item) => <option key={item} value={item}>{item}</option>)}</select><select aria-label={labels.customer} onChange={(event) => setCustomer(event.target.value)} value={customer}><option value="all">{language === "ar" ? "كل العملاء" : "All customers"}</option>{customers.map(([key, entry]) => <option key={key} value={key}>{entry?.name || entry?.email || entry?.phone}</option>)}</select><label className="sales-date-filter"><span>{language === "ar" ? "من" : "From"}</span><input onChange={(event) => setFrom(event.target.value)} type="date" value={from} /></label><label className="sales-date-filter"><span>{language === "ar" ? "إلى" : "To"}</span><input onChange={(event) => setTo(event.target.value)} type="date" value={to} /></label></SalesToolbar>
      {filtered.length ? <AdminOrdersTable canDelete={canUseSalesAction(user, "deleteOrder")} canUpdateStatus={canUseSalesAction(user, "updateOrder")} currency={company?.settings?.currency} employees={employees} language={language} locale={company?.settings?.locale} onAssignEmployee={onAssignEmployee} onDeleteOrder={onDeleteOrder} onStatusChange={onStatusChange} onViewOrder={setSelectedOrder} orders={filtered} products={products} t={t} /> : <EmptySalesState illustration="orders" labels={labels} title={language === "ar" ? "لا توجد طلبات بعد" : "No orders yet"} />}
    </section>
    {selectedOrder && <OrderDetailDialog company={company} employees={employees} labels={labels} language={language} onClose={() => setSelectedOrder(null)} order={selectedOrder} />}
  </>;
}
...
function AnalyticsPage({ company, language, labels, orders, products }) {
  ...
  return <>
    <div className="sales-kpi-grid sales-overview-kpis"><SalesMetric icon={WalletCards} label={labels.sales} value={money(data.metrics.totalSales)} /><SalesMetric icon={ShoppingCart} label={labels.orders} value={data.metrics.orders} /><SalesMetric icon={RefreshCcw} label={language === "ar" ? "الطلبات المعلقة" : "Pending orders"} value={data.metrics.pendingOrders} /><SalesMetric icon={Check} label={language === "ar" ? "الطلبات المكتملة" : "Completed orders"} value={data.metrics.completedOrders} /><SalesMetric icon={X} label={language === "ar" ? "الطلبات الملغاة" : "Cancelled orders"} value={data.metrics.cancelledOrders} /><SalesMetric icon={BarChart3} label={labels.average} value={money(data.metrics.averageOrderValue)} /></div>
    ...
    <section className="sales-data-card sales-orders-over-time"><div className="sales-card-heading"><div><h2>{language === "ar" ? "الطلبات بمرور الوقت" : "Orders over time"}</h2><span>{labels.dateRange}</span></div></div>{data.ordersOverTime.length ? <div className="sales-order-bars">{data.ordersOverTime.map((item) => <div key={item.key}><span>{item.key}</span><i style={{ "--sales-order-count": item.orders }} /><strong>{item.orders}</strong></div>)}</div> : <EmptySalesState illustration="orders" labels={labels} />}</section>
