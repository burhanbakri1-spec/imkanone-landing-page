/**
 * Staging-only Velvet workbook age population (age field only).
 *
 * Default: dry-run (no writes).
 * Apply:
 *   node scripts/update-velvet-workbook-age-staging.js --apply
 *
 * Requires:
 ...
 *   CONFIRM_VELVET_STAGING_AGE=kids-velvet@eb_catalog_test   (for --apply)
 */
...
import {
  ...
  VELVET_SITE_ID,
  getSourceProductId,
  isCleanVelvetWorkbookAgePlan,
  parseVelvetWorkbookAgeRows,
  planVelvetWorkbookAgeUpdate,
  summarizeVelvetWorkbookAgeTags,
  summarizeVelvetWorkbookAgeVerification,
...
`;
  const outPath = path.join(process.env.TEMP || "/tmp", `velvet-workbook-age-${Date.now()}.json`);
...
if (apply) {
  ...
  if (process.env.CONFIRM_VELVET_STAGING_AGE !== "kids-velvet@eb_catalog_test") {
    fail("CONFIRM_VELVET_STAGING_AGE=kids-velvet@eb_catalog_test is required for --apply.");
  }
}

const classifiedRows = loadWorkbookClassifiedRows(workbookPath);
const workbookProducts = parseVelvetWorkbookAgeRows(classifiedRows);
if (workbookProducts.length !== 434) {
  fail(`Expected 434 workbook products with age metadata, found ${workbookProducts.length}.`);
}
...
const preApplyReport = {
  ...
  siteId,
  workbookPath,
  UNIQUE_AGE_TAGS: plan.UNIQUE_AGE_TAGS,
  PRODUCTS_MATCHED: plan.MATCHED_TO_WORKBOOK,
  PRODUCTS_WITH_AGE: plan.PRODUCTS_WITH_AGE,
  PRODUCTS_WITHOUT_AGE: plan.PRODUCTS_WITHOUT_AGE,
  UNRESOLVED_AGE_TAGS: plan.UNRESOLVED_AGE_TAGS,
  ...
  ERRORS: plan.ERRORS,
  CLEAN: plan.CLEAN,
  UNIQUE_RESOLVED_AGE_VALUES: summarizeVelvetWorkbookAgeTags(workbookProducts).uniqueResolvedAgeValues,
  SINGLE_AGE_PRODUCTS: plan.SINGLE_AGE_PRODUCTS,
  MULTI_AGE_PRODUCTS: plan.MULTI_AGE_PRODUCTS,
  PIPE_DELIMITED_VALUES_PERSISTED: plan.wouldUpdate.some((item) => typeof item.targetAge === "string" && item.targetAge.includes("|")),
};
console.log(JSON.stringify(preApplyReport, null, 2));

if (!apply) {
  process.exit(isCleanVelvetWorkbookAgePlan(plan) ? 0 : 2);
}

if (!isCleanVelvetWorkbookAgePlan(plan)) {
...
const productBackupFile = path.join(backupDir, `velvet-kids-velvet-age-products-${stamp}.json`);
...
await writeFile(
  ...
  `${JSON.stringify({
    ...
    products: existingProducts.map((product) => ({
      ...
      age: product.age ?? product.data?.age ?? null,
...
for (const item of plan.wouldUpdate) {
  await request(`${apiUrl}/api/products/${encodeURIComponent(item.productId)}`, {
    method: "PUT",
    body: JSON.stringify({ age: item.targetAge }),
  });
  updatedProducts += 1;
}
...
const verification = summarizeVelvetWorkbookAgeVerification({
...
console.log(JSON.stringify({
  ...
  TOTAL_PRODUCTS: afterProducts.length,
  MATCHED_PRODUCTS: verification.matched,
  UNIQUE_AGE_TAGS: plan.UNIQUE_AGE_TAGS,
  AGE_VALUES_CREATED_OR_USED: [...new Set(workbookProducts.flatMap((row) => row.targetAge))],
  SINGLE_AGE_PRODUCTS: verification.singleAge ?? null,
  MULTI_AGE_PRODUCTS: verification.multiAge ?? null,
  PIPE_DELIMITED_VALUES_PERSISTED: afterProducts.some((product) => typeof product?.age === "string" && product.age.includes("|")),
  PRODUCTS_UPDATED: updatedProducts,
  PRODUCTS_ALREADY_CORRECT: verification.alreadyCorrect,
  PRODUCTS_WITHOUT_AGE: verification.withoutAge,
  UNRESOLVED_PRODUCTS: verification.unresolved,
